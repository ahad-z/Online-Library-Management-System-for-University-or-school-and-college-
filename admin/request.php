...
<body>
	...
	<?php
	if(isset($_SESSION['no_msg'])){

		echo '<div style= "margin-left:500px; width:500px;text-align:center;" class="alert alert-info" >'.$_SESSION['no_msg'].'</div>';

        unset($_SESSION['no_msg']);
...
if (isset($_SESSION['userName']))
{
    ...
    $req_book = mysqli_query($link, "SELECT issue_book.`username`,issue_book.`rolls`, books.`bid`, books.`name` , books.`author`, books.`edition`,books.`status`, books.`quantity`, books.`department` FROM books INNER JOIN issue_book ON books.bid = issue_book.bid WHERE issue_book.approve = ''");

    if (mysqli_num_rows($req_book) == 0)
    {

        $_SESSION['no_msg'] = "There is no  Request for Book!";
